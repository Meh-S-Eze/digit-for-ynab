...
class ListScheduledTransactionsTool extends MCPTool<ListScheduledTransactionsInput> {
    ...
    async execute(input: ListScheduledTransactionsInput): Promise<ListScheduledTransactionsResult | string> {
        ...
        try {
            ...
            const result: ListScheduledTransactionsResult = scheduledTransactions
                .filter(t => !t.deleted)
                .map(t => ({
                    id: t.id,
