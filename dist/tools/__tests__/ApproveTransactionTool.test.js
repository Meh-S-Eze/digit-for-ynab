...
describe('ApproveTransactionTool', () => {
    ...
    it('should approve a transaction successfully', async () => {
        ...
        const parsed = JSON.parse(result);
        expect(mockApi.transactions.updateTransaction).toHaveBeenCalledWith('test-budget', 'txn-1', { transaction: { approved: true } });
    ...
    it('should un-approve a transaction successfully', async () => {
        ...
        const result = await tool.execute(input);
        const parsed = JSON.parse(result);
        expect(mockApi.transactions.updateTransaction).toHaveBeenCalledWith('test-budget', 'txn-1', { transaction: { approved: false } });
        expect(parsed.success).toBe(true);
    });
