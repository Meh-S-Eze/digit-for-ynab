 ...
 */
class UpdateMultipleTransactionsTool extends MCPTool {
    ...
    async execute(input) {
        ...
        try {
            ...
            const transformedTransactions = input.transactions.map(tx => {
                ...
                if (tx.accountId)
                    transformed.account_id = tx.accountId;
                if (tx.date)
                    transformed.date = tx.date;
                if (tx.amount !== undefined)
                    transformed.amount = Math.round(tx.amount * 1000); // Convert to milliunits
                if (tx.payeeName !== undefined)
                    transformed.payee_name = tx.payeeName;
                if (tx.payeeId !== undefined)
                    transformed.payee_id = tx.payeeId;
                ...
                if (tx.memo !== undefined)
                    transformed.memo = tx.memo;
                if (tx.cleared)
                    transformed.cleared = tx.cleared;
                if (tx.approved !== undefined)
                    transformed.approved = tx.approved;
                if (tx.flagColor !== undefined)
                    transformed.flag_color = tx.flagColor;
                return transformed;
            });
            const data = {
