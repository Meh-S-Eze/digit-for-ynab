 ...
 *   transaction total.
 */
class CreateSplitTransactionTool extends SerializingMCPTool {
    name = "create_split_transaction";
    description = "Update an existing transaction so it becomes a split transaction divided into multiple sub-transactions across different categories.";
    schema = {
        budgetId: z
            .string()
        ...
        transactionId: z
            .string()
            .describe("The ID of the existing transaction to split. Obtain this from create_transaction, create_multiple_transactions, or get_unapproved_transactions."),
        accountId: z
            .string()
            .optional()
        ...
        date: z
            ...
            .regex(/^\d{4}-\d{2}-\d{2}$/)
            .optional()
            .describe("The transaction date in YYYY-MM-DD format (optional, defaults to existing)."),
        amount: z
            .number()
            .optional()
            .describe("The total transaction amount in dollars. If provided, it must match the sum of subtransactions."),
        payeeId: z
            .string()
            .nullable()
            .optional()
        ...
        subtransactions: z
            ...
            amount: z
                .number()
                .describe("The subtransaction amount in dollars (same sign as parent)."),
            memo: z.string().nullable().optional(),
            payeeId: z.string().nullable().optional(),
            payeeName: z.string().nullable().optional(),
        }))
            .describe("Array of subtransactions that make up the split. The sum of their amounts must equal the parent transaction total if 'amount' is provided."),
    };
    api;
    constructor() {
        super();
    ...
    async executeInternal(input) {
        ...
        const subTotal = input.subtransactions.reduce((sum, sub) => sum + sub.amount, 0);
        ...
        const totalAmount = input.amount !== undefined ? input.amount : subTotal;
        try {
            logger.info(`Updating transaction ${input.transactionId} to split for budget ${input.budgetId}`);
            // Transform subtransactions to YNAB format
            const transformedSubtransactions = input.subtransactions.map((sub) => ({
            ...
            if (input.accountId)
                transactionData.account_id = input.accountId;
            if (input.date)
                transactionData.date = input.date;
            ...
            if (input.memo !== undefined)
                transactionData.memo = input.memo || null;
            const data = {
            ...
            logger.error(`Error updating split transaction for budget ${input.budgetId}:`);
            logger.error(JSON.stringify(error, null, 2));
            return `Error updating split transaction for budget ${input.budgetId}: ${JSON.stringify(error)}`;
        }
    }
}
